...
class PanelApiService(config: PanelConfig) {
    ...
    private fun extractMsgFromText(text: String): String {
        return try {
            json.decodeFromString<org.example.project.data.ApiErrorBody>(text).msg
        } catch (_: Exception) { "" }
    }
    ...
    suspend fun isLogin(sessionCookie: String? = null): Result<Boolean> {
        return try {
            val resp = client.get("/api/user/is_login", sessionCookie = sessionCookie)
            // 该接口不论是否登录都返回 200，只是 data 不同
            val body = resp.bodyAsText()
            Result.success(body.contains("\"is_login\":true") || body.contains("\"is_login\": true"))
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
    ...
    suspend fun setWebsiteStatus(id: Long, status: Boolean): Result<Unit> {
        return try {
            val body = json.encodeToString(WebsiteStatusRequest(status = status))
    ...
    suspend fun deleteWebsite(id: Long, deletePath: Boolean = false, deleteDb: Boolean = false): Result<Unit> {
        return try {
            val body = json.encodeToString(DeleteWebsiteRequest(path = deletePath, db = deleteDb))
    ...
    suspend fun setCronTaskStatus(id: Long, status: Boolean): Result<Unit> {
        return try {
            val body = json.encodeToString(CronStatusRequest(status))
    ...
    /** 获取应用列表 */
    suspend fun getApps(page: Int = 1, limit: Int = 20, category: String = ""): Result<AppListResponse> {
        return try {
    ...
    suspend fun installApp(slug: String, channel: String = "stable"): Result<Unit> {
        return try {
            val body = """{"slug":"$slug","channel":"$channel"}"""
    ...
    suspend fun updateCronTask(id: Long, req: UpdateCronRequest): Result<Unit> {
        return try {
            val resp = client.put("/api/cron/$id", json.encodeToString(req))
            if (resp.status.isSuccess()) Result.success(Unit)
            else Result.failure(Exception(resp.bodyAsText()))
        } catch (e: Exception) { Result.failure(e) }
    }
    ...
    suspend fun updateDatabaseUser(id: Long, req: UpdateDatabaseUserRequest): Result<Unit> {
        return try {
            val resp = client.put("/api/database_user/$id", json.encodeToString(req))
            if (resp.status.isSuccess()) Result.success(Unit)
            else { val err = resp.bodyAsText(); Result.failure(Exception("更新失败: $err")) }
        } catch (e: Exception) { Result.failure(e) }
    }
    ...
    suspend fun updateUsername(id: Long, username: String): Result<Unit> {
        return try {
            val body = """{"username":${json.encodeToString(username)}}"""
    ...
    suspend fun updatePassword(id: Long, password: String): Result<Unit> {
        return try {
            val body = """{"password":${json.encodeToString(password)}}"""
    ...
    suspend fun updateEmail(id: Long, email: String): Result<Unit> {
        return try {
            val body = """{"email":${json.encodeToString(email)}}"""
            ...
            client.connectWs("/api/ws/pty") {
                ...
                for (frame in incoming) {
                    when (frame) {
                        ...
                        is Frame.Binary -> onOutput(frame.data.toString(Charsets.UTF_8))
                        else -> {}
                    }
                }
                senderJob.cancel()
