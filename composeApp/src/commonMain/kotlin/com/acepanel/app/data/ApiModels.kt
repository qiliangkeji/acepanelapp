...
object EpochMillisSerializer : KSerializer<Long> {
    ...
    override fun deserialize(decoder: Decoder): Long {
        ...
        return content.toLongOrNull() ?: parseRfc3339Millis(content)
    }
    ...
    private fun parseRfc3339Millis(value: String): Long {
        return runCatching {
            val text = value.trim()
            if (text.length < 19) return 0L
            val year = text.substring(0, 4).toInt()
            val month = text.substring(5, 7).toInt()
            val day = text.substring(8, 10).toInt()
            val timeStart = text.indexOfFirst { it == 'T' || it == ' ' }
            if (timeStart < 0 || timeStart + 8 >= text.length) return 0L
            val hour = text.substring(timeStart + 1, timeStart + 3).toInt()
            val minute = text.substring(timeStart + 4, timeStart + 6).toInt()
            val second = text.substring(timeStart + 7, timeStart + 9).toInt()

            val afterSecond = text.substring(timeStart + 9)
            val fraction = if (afterSecond.startsWith(".")) {
                afterSecond.drop(1).takeWhile { it.isDigit() }.take(3).padEnd(3, '0').toIntOrNull() ?: 0
            } else {
                0
            }

            val offsetIndex = text.indexOfAny(charArrayOf('Z', '+', '-'), startIndex = timeStart + 9)
            val offsetMinutes = if (offsetIndex < 0 || text[offsetIndex] == 'Z') {
                0
            } else {
                val sign = if (text[offsetIndex] == '+') 1 else -1
                val offset = text.substring(offsetIndex + 1)
                val offsetHour = offset.take(2).toInt()
                val offsetMinute = offset.drop(3).take(2).toIntOrNull() ?: 0
                sign * (offsetHour * 60 + offsetMinute)
            }

            val localMillis = daysFromCivil(year, month, day) * 86_400_000L +
                hour * 3_600_000L + minute * 60_000L + second * 1_000L + fraction
            localMillis - offsetMinutes * 60_000L
        }.getOrDefault(0L)
    }
...
object FlexibleBooleanSerializer : KSerializer<Boolean> {
    ...
    override fun deserialize(decoder: Decoder): Boolean {
        ...
        return when (primitive.content.trim().lowercase()) {
            "true", "1", "yes", "valid", "active", "running", "enabled", "online" -> true
            else -> false
        }
    }
...
data class CronTaskApiItem(
    ...
    val time: String = "",
    val config: CronConfigData = CronConfigData(),
    val shell: String = "",
    val log: String = ""
)

@Serializable
data class CronConfigData(
    val type: String = "",
    val flock: Boolean = false,
...
data class PanelSettingData(
    ...
    val two_fa: Boolean = false,
    val lifetime: Int = 60,
    val ip_header: String = "",
    @Serializable(with = StringListOrEmptySerializer::class)
    val bind_domain: List<String> = emptyList(),
    @Serializable(with = StringListOrEmptySerializer::class)
    val bind_ip: List<String> = emptyList(),
    @Serializable(with = StringListOrEmptySerializer::class)
    val bind_ua: List<String> = emptyList(),
    val website_path: String = "",
    val backup_path: String = "",
    val backup_format: String = "tar.xz",
    val project_path: String = "",
    val container_sock: String = "",
    @Serializable(with = StringListOrEmptySerializer::class)
    val hidden_menu: List<String> = emptyList(),
    val custom_logo: String = "",
    val ipdb_type: String = "",
    val ipdb_url: String = "",
    val ipdb_path: String = "",
    val tls: String = "off",
    @Serializable(with = StringListOrEmptySerializer::class)
    val public_ip: List<String> = emptyList(),
    val cert: String = "",
    val key: String = "",
    val port: Int = 8888
)
...
data class ProjectListItem(
    ...
    val exec_start_pre: String = "",
    val exec_start_post: String = "",
    val exec_start: String = "",
    val exec_stop: String = "",
    val exec_reload: String = "",
    val user: String = "",
    val restart: String = "",
    val restart_sec: String = "",
    val restart_max: Int = 0,
    val timeout_start_sec: Int = 0,
    val timeout_stop_sec: Int = 0,
    val environments: List<ProjectEnvItem> = emptyList(),
    ...
    val memory_limit: Double = 0.0,
    val cpu_quota: Double = 0.0,
    val no_new_privileges: Boolean = false,
    val protect_tmp: Boolean = false,
    val protect_home: Boolean = false,
    val protect_system: String = "",
    @Serializable(with = StringListOrEmptySerializer::class)
    val read_write_paths: List<String> = emptyList(),
    @Serializable(with = StringListOrEmptySerializer::class)
    val read_only_paths: List<String> = emptyList(),
    val status: String = "",
...
@Serializable
data class FileRemoteDownloadRequest(
    val path: String,
    val url: String
)

@Serializable
data class FilePermissionRequest(
    val path: String,
    val mode: String,
    val owner: String,
    val group: String
...
@Serializable
data class FileCompressRequest(
    val dir: String,
...
@Serializable
data class FileUnCompressRequest(
    val file: String,
    val path: String
)
...
@Serializable
data class UpdateBackupStorageRequest(
    val id: Long,
    val type: String,
    val name: String,
    val info: BackupStorageInfo
...
@Serializable
data class UpdateAcmeAccountRequest(
    val id: Long,
    val ca: String,
    val email: String,
    val key_type: String,
...
@Serializable
data class UpdateDnsProviderRequest(
    val id: Long,
    val type: String,
    val name: String,
    val data: DnsParamData
...
@Serializable
data class MonitorLoadHistory(
    val load1: List<Double> = emptyList(),
    val load5: List<Double> = emptyList(),
    val load15: List<Double> = emptyList()
)

@Serializable
data class MonitorCpuHistory(
    val percent: List<String> = emptyList()
)

@Serializable
data class MonitorMemHistory(
    val total: String = "",
...
@Serializable
data class MonitorNetHistory(
    val name: String = "",
    val sent: List<String> = emptyList(),
    val recv: List<String> = emptyList(),
    val tx: List<String> = emptyList(),
    val rx: List<String> = emptyList()
)

@Serializable
data class MonitorDiskIoHistory(
    val name: String = "",
    val read_bytes: List<String> = emptyList(),
    val write_bytes: List<String> = emptyList(),
    val read_speed: List<String> = emptyList(),
    val write_speed: List<String> = emptyList()
)

@Serializable
data class MonitorHistoryResponse(
    val times: List<String> = emptyList(),
    val load: MonitorLoadHistory = MonitorLoadHistory(),
    val cpu: MonitorCpuHistory = MonitorCpuHistory(),
    val mem: MonitorMemHistory = MonitorMemHistory(),
    val swap: MonitorSwapHistory = MonitorSwapHistory(),
    val net: List<MonitorNetHistory> = emptyList(),
    val disk_io: List<MonitorDiskIoHistory> = emptyList(),
    val top_processes: List<TopProcessesUsage> = emptyList()
)
